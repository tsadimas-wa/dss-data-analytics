...
Το **Contract Net Protocol (CNP)** (Smith, 1980) είναι το πιο κλασικό πρωτόκολλο **συνεργατικής κατανομής εργασιών** σε MAS — εμπνέεται από επιχειρηματικές διαδικασίες ανάθεσης έργων.

<div class="columns">
<div>

**Ρόλοι:**
- **Manager (Διαχειριστής):** Έχει μια εργασία που δεν μπορεί/θέλει να εκτελέσει μόνος του
- **Contractor (Ανάδοχος):** Έχει ικανότητα εκτέλεσης της εργασίας

**Βήματα πρωτοκόλλου:**

```
1. ANNOUNCEMENT (Ανακοίνωση)
   Manager → Broadcast:
   «Ζητείται εκτέλεση εργασίας T»
   (περιγραφή, απαιτήσεις, deadline)

2. BIDDING (Υποβολή Προσφορών)
   Contractor → Manager:
   «Μπορώ να εκτελέσω T με κόστος C»
   (μόνο αν έχει τις απαιτούμενες ικανότητες)

3. AWARDING (Ανάθεση)
   Manager → Best Contractor:
   «Αναθέτω σε εσένα την εργασία T»

4. EXECUTION & REPORT (Εκτέλεση & Αναφορά)
   Contractor → εκτελεί T
   Contractor → Manager: αποτέλεσμα
```
...
Manager → Announcement (broadcast)
  ...
  Contractor B: Bid(30)
     ↓
Manager → Award → Contractor B
     ↓
Contractor B εκτελεί
     ↓
Manager ← Report: Done
```
...
| Ιδιότητα | Τιμή |
|---|---|
| Αποκέντρωση | Υψηλή (δεν απαιτεί κεντρ. έλεγχο) |
| Ευελιξία | Δυναμική ανάθεση βάσει ικανοτήτων |
| Υπερ-φόρτωση δικτύου | Μεσαία (broadcasting) |
| Εφαρμογή | Supply chain, grid computing, robotics |

> **Παράδειγμα:** Σε MAS εφοδιαστικής αλυσίδας, ο *Πράκτορας Παραγγελίας* εκδίδει CNP-ανακοίνωση για μεταφορά εμπορευμάτων — οι *Πράκτορες Μεταφοράς* υποβάλλουν προσφορές βάσει διαθεσιμότητας & κόστους.
...
Η εμφάνιση των **Μεγάλων Γλωσσικών Μοντέλων (LLMs)** ανέτρεψε την παραδοσιακή ανάπτυξη πρακτόρων. Σήμερα, ο «εγκέφαλος» ενός πράκτορα είναι ένα LLM (GPT-4, Claude, Gemini κ.λπ.) που συλλογίζεται και σχεδιάζει με φυσική γλώσσα.
...
| Στοιχείο | Ρόλος |
|---|---|
| **LLM (Brain)** | Κατανόηση, σχεδιασμός, λήψη αποφάσεων σε φυσική γλώσσα |
| **Tools (Εργαλεία)** | Αναζήτηση ιστού, εκτέλεση κώδικα, κλήση APIs, βάσεις δεδομένων |
| **Memory (Μνήμη)** | *Short-term*: context window · *Long-term*: vector database (RAG) |
| **Planning** | Διάσπαση στόχου σε υπο-εργασίες — ReAct, CoT, Tree-of-Thoughts |

**Πλαίσιο ReAct (Reason + Act):**
```
Στόχος: «Βρες την τρέχουσα τιμή μετοχής AAPL»

Thought: Πρέπει να αναζητήσω online
Act:     search("AAPL stock price today")
Obs:     «Apple: $189.50»
Thought: Έχω την απάντηση
Answer:  «Η τιμή της AAPL είναι $189.50»
```

</div>
<div>

**Σύγχρονα συστήματα & εργαλεία:**

| Σύστημα | Περιγραφή |
|---|---|
| **AutoGPT** | Αυτόνομος πράκτορας — επιδιώκει μακροπρόθεσμους στόχους χωρίς παρέμβαση |
| **Devin** | AI software engineer — γράφει & δοκιμάζει κώδικα αυτόνομα |
| **CrewAI / LangGraph** | Multi-agent frameworks — πολλοί LLM-πράκτορες σε ρόλους (π.χ. Researcher, Writer, Critic) |
| **OpenAI Agents SDK** | Πλαίσιο ανάπτυξης πρακτόρων με handoffs & tools |

**Σύνδεση με κλασικές αρχιτεκτονικές:**

| Κλασική έννοια | LLM-Agent αντίστοιχο |
|---|---|
| Beliefs | Context + Retrieved memories |
| Intentions | Current plan / task list |
| Tools | Actuators |
| Sensors | Web search, file read, API calls |

> **Πρόκληση:** Αξιοπιστία (hallucinations), ασφάλεια (prompt injection), κόστος (API calls), επαναληψιμότητα αποτελεσμάτων.
...
```
Γράψε κώδικα → Τρέξε tests
       ↓ αποτυχία;
  Διάβασε σφάλμα → Διόρθωσε → Επανάλαβε ↑
```
