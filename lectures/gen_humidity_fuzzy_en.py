...
ax.plot(H, mu_low,    color='#2196F3', lw=2.5, label='Low     — trap(0,0,30,50)')
ax.plot(H, mu_medium, color='#FF9800', lw=2.5, label='Medium  — tri(35,55,75)')
ax.plot(H, mu_high,   color='#4CAF50', lw=2.5, label='High    — trap(60,80,100,100)')
...
ax.annotate(f'mu_Medium = {mu_medium_72:.2f}',
...
ax.annotate(f'mu_High = {mu_high_72:.2f}',
            xy=(h_val, mu_high_72), xytext=(h_val + 4, mu_high_72 + 0.12),
            arrowprops=dict(arrowstyle='->', color='#4CAF50', lw=1.5),
            fontsize=11, color='#4CAF50', fontweight='bold')

ax.annotate('mu_Low = 0.00\n(out of range)',
            xy=(h_val, 0.0), xytext=(h_val + 4, 0.10),
            arrowprops=dict(arrowstyle='->', color='#2196F3', lw=1.5),
            fontsize=10, color='#2196F3')

ax.set_xlabel('Humidity H (%)', fontsize=13)
ax.set_ylabel('Membership degree mu', fontsize=13)
ax.set_title('Fuzzification — Humidity H = 72%', fontsize=14, fontweight='bold')
ax.set_xlim(0, 100)
...
fig.tight_layout()
out = 'lectures_material/humidity_fuzzification_en.png'
