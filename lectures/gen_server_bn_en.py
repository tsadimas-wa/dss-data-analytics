...
draw_node(ax, *pos['D'], 'D — DDoS Attack',       'prior: P(D=T) = 0.02', C_ROOT)
draw_node(ax, *pos['H'], 'H — Hardware Failure',   'prior: P(H=T) = 0.05', C_ROOT)
draw_node(ax, *pos['A'], 'A — Slow Response',      'P(A | D, H)  ← CPT 2', C_INTER)
draw_node(ax, *pos['P'], 'P — User Complaints',
...
ax.text(4.0, 1.15, 'Posterior — given P = T', ha='center',
...
legend = [
    mpatches.Patch(color=C_ROOT,  label='Root — known prior'),
    mpatches.Patch(color=C_INTER, label='Intermediate — CPT 2'),
...
fig1.savefig('lectures_material/server_bn_graph_en.png', dpi=150, bbox_inches='tight')
print('Saved: server_bn_graph_en.png')
...
# ── CPT 1 ─────────────────────────────────────────────────────────────────────
y = 0.92
y = table(ax2, 'CPT 1 — Priors (independent variables)',
          ['Variable', 'P( = T)', 'P( = F)'],
          [['D — DDoS Attack',      '0.02', '0.98'],
           ['H — Hardware Failure', '0.05', '0.95']],
          COLS['root'],
...
ax2.set_title('Conditional Probability Tables (CPT)', fontsize=13,
              fontweight='bold', pad=10)

fig2.tight_layout()
fig2.savefig('lectures_material/server_bn_cpts_en.png', dpi=150, bbox_inches='tight')
print('Saved: server_bn_cpts_en.png')
