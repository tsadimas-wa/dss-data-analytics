...
nodes = {
    'history':  ('Payment\nHistory',        (1.8, 5.4), '#1565C0',
                 'P(Good)=0.70\nP(Bad)=0.30'),
    'income':   ('Income',                  (8.2, 5.4), '#1565C0',
                 'P(High)=0.45\nP(Low)=0.55'),
    'credit':   ('Credit\nScore',           (3.0, 3.4), '#6A1B9A',
                 'P(High|Good)=0.85\nP(High|Bad)=0.20'),
    'debt':     ('Debt\nRatio',             (7.2, 3.4), '#6A1B9A',
                 'P(Low|High)=0.78\nP(Low|Low)=0.30'),
    'risk':     ('Loan\nRisk',              (5.1, 1.6), '#B71C1C',
                 'see CPT →'),
    'approval': ('Approval\nDecision',      (5.1, 0.3), '#2E7D32',
                 'P(Appr.|Low)=0.90\nP(Appr.|High)=0.05'),
}
...
# CPT mini-table next to Risk node
cpt_x, cpt_y = 8.5, 1.6
ax.text(cpt_x, cpt_y + 0.7, 'CPT — P(Risk = High)', fontsize=12,
        ha='center', fontweight='bold', color='#B71C1C')
rows = [
    ('Credit Score', 'Debt Ratio', 'P'),
    ('Low',          'High',       '0.92'),
    ('Low',          'Low',        '0.65'),
    ('High',         'High',       '0.40'),
    ('High',         'Low',        '0.08'),
...
# Legend
legend_items = [
    mpatches.Patch(color='#1565C0', label='Observed variable (root)'),
    mpatches.Patch(color='#6A1B9A', label='Intermediate variable'),
    mpatches.Patch(color='#B71C1C', label='Risk node'),
    mpatches.Patch(color='#2E7D32', label='Output (decision)'),
]
ax.legend(handles=legend_items, loc='lower left', fontsize=12,
          framealpha=0.9, edgecolor='#ccc')

# ax.set_title('Bayesian Network — Credit Risk Assessment', fontsize=18,
#              fontweight='bold', pad=10)

fig.tight_layout()
out = 'lectures_material/bayes_credit_net_en.png'
