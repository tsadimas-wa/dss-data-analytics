...
> **Gini Impurity:** Μετράει πόσο «ανακατεμένος» είναι ένας κόμβος. **Gini = 0** → ο κόμβος έχει μία μόνο κλάση (τέλειος). **Gini = 0,5** → 50/50 ανάμειξη (χειρότερος). Τύπος: $1 - \sum p_i^2$. Επιλέγουμε το split που **μειώνει περισσότερο** το Gini.

**Πώς βρίσκουμε το κατώφλι;** Ο αλγόριθμος δοκιμάζει *όλες* τις δυνατές τιμές για κάθε αριθμητικό χαρακτηριστικό — τις μέσες τιμές μεταξύ διαδοχικών τιμών του dataset. Για το **Tenure** (τιμές: 1,3,8,10,24,36,48,60) δοκιμάζει: < 2, < 5.5, < 9, **< 12**, < 30, < 42, < 54 — και κρατά το κατώφλι με το μικρότερο Gini. Για το **MonthlyCharges** (45,50,55,60,78,85,90,92) ομοίως δοκιμάζει: < 47.5, < 52.5, ... **> 70** (= < 72.5), κ.ο.κ. Τα κατώφλια που βλέπουμε παρακάτω είναι ήδη τα **νικητήρια** για κάθε χαρακτηριστικό.
...
> ⚠️ **Γιατί η επόμενη εικόνα είναι διαφορετική;** Τα 8 δείγματα είναι υποδειγματικά — τυχαία «τέλεια» χωρίζονται από ένα μόνο χαρακτηριστικό. Σε πραγματικό dataset χιλιάδων πελατών καμία μεταβλητή δεν χωρίζει τέλεια: τα φύλλα δεν είναι καθαρά, οπότε ο αλγόριθμος συνεχίζει να σπάει κόμβους — και μπορεί να αναδείξει διαφορετική ρίζα (π.χ. Tenure). Το τελικό δέντρο της επόμενης διαφάνειας προέκυψε από εκπαίδευση σε **μεγαλύτερο dataset**.
