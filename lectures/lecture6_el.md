...
<!-- _class: xxsmall -->
# 2ο Παράδειγμα: Superstore Dataset (Πρόβλεψη Κερδοφορίας)

**Πλαίσιο:** Ένα κατάστημα λιανικής (Superstore) θέλει να προβλέψει αν μια παραγγελία θα είναι **Κερδοφόρα (Profit > 0)** ή **Ζημιογόνα (Profit ≤ 0)**, με βάση τα χαρακτηριστικά της, ώστε να προσαρμόσει την πολιτική εκπτώσεων.

**Ενδεικτικό Dataset (10 Παραγγελίες):**

| OrderID | Category | Sub-Category | Discount (%) | Shipping Cost | **Profitable** |
|---|---|---|---|---|---|
| O1 | Furniture | Tables | 30% | 50€ | **Όχι (🔴)** |
| O2 | Technology | Phones | 0% | 10€ | **Ναι (🟢)** |
| O3 | Office Supplies | Binders | 20% | 5€ | **Ναι (🟢)** |
| O4 | Furniture | Chairs | 50% | 20€ | **Όχι (🔴)** |
| O5 | Technology | Accessories | 20% | 15€ | **Ναι (🟢)** |
| O6 | Office Supplies | Paper | 70% | 5€ | **Όχι (🔴)** |

> **Στόχος:** Ο αλγόριθμος αναζητά το χαρακτηριστικό (π.χ. Category ή Discount) που διαχωρίζει καλύτερα τις κερδοφόρες από τις ζημιογόνες παραγγελίες. Το **Discount** είναι το πιο ισχυρό κριτήριο: οι μεγάλες εκπτώσεις συνήθως "τρώνε" το περιθώριο κέρδους!

---

<!-- _class: xxsmall -->
# Χτίζοντας το Δέντρο (Superstore) — Το αποτέλεσμα

Αν τρέξουμε τον αλγόριθμο (π.χ. CART) στο Superstore dataset, η διαδικασία διαχωρισμού (splits) δημιουργεί ένα δέντρο σαν αυτό:

<div class="columns">
<div>

**Πώς διαβάζεται το δέντρο:**
1. **Ρίζα (Root):** Ο αλγόριθμος βρίσκει ότι το **Discount > 25%** είναι ο καλύτερος διαχωριστής (υψηλότερο Gini Gain). Οι παραγγελίες με τεράστια έκπτωση είναι σχεδόν πάντα ζημιογόνες!
2. **Αριστερός κλάδος (Discount ≤ 25%):** Οι περισσότερες παραγγελίες εδώ έχουν κέρδος. Το δέντρο κάνει νέο split: *Μήπως η κατηγορία είναι Furniture?* Τα έπιπλα έχουν υψηλό κόστος αποστολής, άρα το περιθώριο είναι μικρό.
3. **Δεξιός κλάδος (Discount > 25%):** Αν η έκπτωση είναι τεράστια (π.χ. 50%), το δέντρο προβλέπει κατευθείαν 🔴 **Ζημία**.

</div>
<div>

**Εννοιολογικό Δέντρο Απόφασης:**
```text
Discount > 25%;
├── ΝΑΙ → 🔴 ΖΗΜΙΑ (Profit < 0)
│         (Μεγάλες εκπτώσεις)
└── ΟΧΙ → Category = Furniture;
           ├── ΝΑΙ → Shipping Cost > 20€;
           │          ├── ΝΑΙ → 🔴 ΖΗΜΙΑ
           │          └── ΟΧΙ → 🟢 ΚΕΡΔΟΣ
           └── ΟΧΙ → 🟢 ΚΕΡΔΟΣ (Profit > 0)
                     (Tech & Office)
...
> **Business Insight:** Το μοντέλο μας λέει στην πράξη: *"Προσοχή στις εκπτώσεις άνω του 25% γενικά, και στα Έπιπλα αν έχουν υψηλά μεταφορικά"*. Αυτοί οι κανόνες εξάγονται **αυτόματα** από τα δεδομένα!
...
<!-- _class: xxsmall -->
# 3ο Παράδειγμα: Πρόβλεψη Επιβίωσης (Τιτανικός - Kaggle)

**Πλαίσιο:** Ίσως το διασημότερο dataset εκμάθησης Μηχανικής Μάθησης (στο Kaggle). Στόχος είναι να προβλέψουμε αν ένας επιβάτης του Τιτανικού **Επέζησε (🟢 Ναι)** ή **Όχι (🔴 Όχι)**, με βάση τα χαρακτηριστικά του.

**Ενδεικτικό Μικρό Dataset (8 Επιβάτες):**

| PassID | Pclass (Θέση) | Sex (Φύλο) | Age (Ηλικία) | **Survived** |
|---|---|---|---|---|
| 1 | 3rd | Male | 22 | **Όχι (🔴)** |
| 2 | 1st | Female | 38 | **Ναι (🟢)** |
| 3 | 3rd | Female | 26 | **Ναι (🟢)** |
...
> Στο σύνολο έχουμε **4 Επιζώντες / 4 Θανόντες**. Αρχικό Gini = $1 - (4/8)^2 - (4/8)^2 = \mathbf{0{,}50}$ (απόλυτη αβεβαιότητα). Ποιο χαρακτηριστικό διαχωρίζει καλύτερα την τύχη τους;

---

<!-- _class: xxsmall -->
# Τιτανικός — Βήμα-βήμα Κατασκευή Δέντρου

<div class="columns">
<div>

**Βήμα 1 — Εύρεση Ρίζας (Root):**
Ο αλγόριθμος δοκιμάζει το **Φύλο (Sex = Female)**:
* **ΝΑΙ (Female):** 3 Επιζώντες, 1 Θανών → $\text{Gini} = 1 - (3/4)^2 - (1/4)^2 = \mathbf{0{,}375}$
* **ΟΧΙ (Male):** 1 Επιζών, 3 Θανόντες → $\text{Gini} = 1 - (1/4)^2 - (3/4)^2 = \mathbf{0{,}375}$

Το Gini πέφτει από $0{,}50$ σε $0{,}375$ (τεράστιο Gini Gain). Το **Φύλο** γίνεται η Ρίζα του δέντρου! Ο αλγόριθμος μόλις "ανακάλυψε" τον κανόνα *"Γυναίκες και παιδιά πρώτα"*.

**Βήμα 2 — Επόμενα Splits:**
* **Αριστερός Κλάδος (Γυναίκες):** Οι περισσότερες επιζούν. Όμως η επιβάτης #8 (3rd class) δεν τα κατάφερε. Το δέντρο κάνει split στο **Pclass (Θέση)**: οι γυναίκες 1ης θέσης σώζονται 100%.
* **Δεξιός Κλάδος (Άνδρες):** Οι περισσότεροι πεθαίνουν. Όμως ο επιβάτης #6 επέζησε γιατί ήταν 5 ετών. Το επόμενο split εδώ θα είναι η **Ηλικία (Age < 10)**: τα αγόρια επέζησαν, οι ενήλικες άνδρες όχι.

</div>
<div>

**Το Δέντρο που Προκύπτει (Κανόνες Εξαγωγής):**
```text
Sex = Female;
├── ΝΑΙ → Pclass = 1st ή 2nd;
│          ├── ΝΑΙ → 🟢 ΕΠΕΖΗΣΕ (Σχεδόν βέβαιο)
│          └── ΟΧΙ → 🔴 Μειωμένη Πιθανότητα (3η Θέση)
└── ΟΧΙ → Age < 10;
           ├── ΝΑΙ → 🟢 ΕΠΕΖΗΣΕ (Αγόρια)
           └── ΟΧΙ → 🔴 ΠΕΘΑΝΕ (Ενήλικες Άνδρες)
```

> **Business / Analytical Insight:** Το δέντρο απόφασης καταφέρνει να "μάθει" ιστορικούς και κοινωνικούς κανόνες (κοινωνική τάξη, προτεραιότητα γυναικών) **εντελώς αυτόματα**, μόνο μέσα από τη μείωση της ακαθαρσίας (Gini) στα δεδομένα!
