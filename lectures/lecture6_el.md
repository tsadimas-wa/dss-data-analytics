...
> Τιμή κοντά στο 0,5 → ο κόμβος είναι **ακάθαρτος** — χρειάζεται split. Στις επόμενες διαφάνειες υπολογίζουμε το Gini Gain για κάθε feature και επιλέγουμε τη ρίζα του δέντρου.
...
$$\text{Variance Reduction} = \text{MSE}_{\text{πριν}} - \left(\frac{n_L}{n}\,\text{MSE}_L + \frac{n_R}{n}\,\text{MSE}_R\right)$$

Όπου $\text{MSE} = \frac{1}{n}\sum_{i=1}^{n}(y_i - \bar{y})^2$, δηλαδή η **μέση τετραγωνική απόκλιση** των τιμών στόχου από τον μέσο όρο τους.
...
* Αριστερά (P1, P2): $\bar{y}_L = 73{,}5$, $\text{MSE}_L = 2{,}25$
* Δεξιά (P3–P6): $\bar{y}_R = 49{,}5$, $\text{MSE}_R = 13{,}25$
...
|---|---|---|
| **MAE** | $\frac{1}{n}\sum|y_i - \hat{y}_i|$ | Μέσο απόλυτο σφάλμα σε € |
| **RMSE** | $\sqrt{\frac{1}{n}\sum(y_i-\hat{y}_i)^2}$ | Τιμωρεί μεγάλα σφάλματα περισσότερο |
| **R²** | $1 - \frac{\text{MSE}_{\text{μοντέλου}}}{\text{MSE}_{\text{βάσης}}}$ | 1 = τέλειο, 0 = μέσος όρος baseline |
