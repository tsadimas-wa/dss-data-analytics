...
| Κλάδος | Σύνθεση | Gini |
|---|---|---|
| ΝΑΙ | C001✗ C003✗ C005✗ → 3Ν/0Ό | $0{,}00$ |
| ΟΧΙ | C002✓ C004✓ C006✗ C007✗ C008✓ → 2Ν/3Ό | $0{,}48$ |
...
| Κλάδος | Σύνθεση | Gini |
|---|---|---|
| ΝΑΙ | C001✗ C003✗ C006✗ C007✗ → 4Ν/0Ό | $0{,}00$ |
| ΟΧΙ | C005✗ C002✓ C004✓ C008✓ → 1Ν/3Ό | $0{,}375$ |
...
| Αλγόριθμος | Κριτήριο | Διάσπαση κατηγορικών | sklearn |
| --- | --- | --- | --- |
| **ID3** (Quinlan, 1986) | Information Gain / Εντροπία | Πολυδύναμη (1 κλάδος / τιμή) | — |
| **CART** (Breiman, 1984) | Gini Gain / Gini Impurity | Πάντα δυαδική | `'gini'` / `'entropy'` |
| **C4.5** (Quinlan, 1993) | GainRatio / Εντροπία | Πολυδύναμη (1 κλάδος / τιμή) | — |

**Γιατί «ID3»;** *Iterative Dichotomizer 3* — χωρίζει (dichotomize) επαναληπτικά τα δεδομένα. Το «3» = τρίτη έκδοση στη σειρά του Quinlan. Βασίστηκε στον αλγόριθμο CLS (Concept Learning System) του Hunt (1966).

**Γιατί «C4.5»;** Ο Quinlan ανέπτυξε σειρά εκδόσεων: ID3 → C4.0 → **C4.5** → C5.0. Το «C» = Classifier, το «4.5» = αριθμός έκδοσης. Ο C4.5 διορθώνει το πρόβλημα του ID3 με το GainRatio και υποστηρίζει συνεχή χαρακτηριστικά.

**Κοινός τύπος:** $\text{Gain} = \text{Ακαθαρσία(γονέα)} - \sum_{\text{κλάδοι}} \tfrac{n_v}{n} \cdot \text{Ακαθαρσία}(v)$

> **Στη sklearn** υλοποιείται **μόνο CART** (ID3 και C4.5 δεν υπάρχουν — διαθέσιμοι στο Weka/Java). Το `criterion` επιλέγει μόνο το μέτρο: `'gini'` → Gini Impurity, `'entropy'` → Εντροπία. Και στις δύο περιπτώσεις ο μηχανισμός παραμένει CART.
...
> Σε Churn, **FN > FP** σε κόστος: καλύτερα μία άσκοπη προσφορά παρά ένας χαμένος πελάτης.
