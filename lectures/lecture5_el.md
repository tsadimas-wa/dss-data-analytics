...
Μετατρέπουμε την ασαφή έξοδο σε **μία κριτή τιμή** με τη μέθοδο centroid (σταθμισμένος μέσος των αντιπροσωπευτικών τιμών):
...
| Μέθοδος | Τύπος | Ερμηνεία |
|---------|-------|----------|
| **min** (Mamdani) | `min(a, b)` | Το αποτέλεσμα δεν μπορεί να είναι ισχυρότερο από τον **αδύναμο κρίκο** |
| **product** | `a × b` | Και οι δύο συνθήκες συνεισφέρουν — μικρή τιμή σε έστω μία **μειώνει πολύ** το αποτέλεσμα |

**Παράδειγμα** με μ_Ζεστή = 1.0 και μ_Μέτρια = 0.15 (από το παράδειγμά μας):

| | min | product |
|--|-----|---------|
| fire R6 | min(1.0, 0.15) = **0.150** | 1.0 × 0.15 = **0.150** |

Με μ_Ζεστή = 0.8 και μ_Μέτρια = 0.15:
...
Έχουμε αθροίσει τα ασαφή αποτελέσματα. Πώς επιλέγουμε **μία κριτή τιμή**;
...
0.15│             ┌───┐           │  (100%)  │
    ...
    0   25    50    75             100
                   ↑               ↑
              Δυνατό (w=0.15)  Πλήρες (w=0.60)
```

| Μέθοδος | Τι επιλέγει | Αποτέλεσμα (παράδειγμα) |
|---------|-------------|------------------------|
| **Centroid** (σταθμ. μέσος) | Κέντρο βάρους ολόκληρης της κατανομής | (0.15×75 + 0.60×100) / 0.75 = **95%** |
| **Mean of Maximum** | Μέση τιμή του υψηλότερου πλατό (μ=0.60) | mean{100} = **100%** |
| **Largest of Max** | Μεγαλύτερη τιμή x με μέγιστο μ | max{100} = **100%** |

> Το **Centroid** είναι η πιο διαδεδομένη επιλογή — δίνει ομαλή, συνεχή έξοδο που αντανακλά *όλους* τους ενεργούς κανόνες, όχι μόνο τον κυρίαρχο.
...
Δύο βασικές αρχιτεκτονικές FIS — διαφέρουν στο **τι παράγει κάθε κανόνας**:

<div class="columns">
<div>

### Mamdani
Κάθε κανόνας παράγει **ασαφές σύνολο** εξόδου.

```
R7: fire=0.60 → "Πλήρες"
    → αποκόπτει την MF "Πλήρες"
      στο επίπεδο 0.60
    → Defuzzification με Centroid
```

✓ Ερμηνεύσιμο — οι κανόνες
  περιγράφουν γλωσσικές τιμές

✗ Βαρύτερος υπολογιστικά

</div>
<div>

### Sugeno
Κάθε κανόνας παράγει **αριθμό** (ή γραμμική συνάρτηση εισόδων).

```
R7: fire=0.60 → output = 100
    (σταθερά ή f(T, H))
    → Defuzzification = σταθμ. μέσος
      0.60×100 / 0.60 = 100
```

✓ Πολύ πιο γρήγορο
✓ Κατάλληλο για Neuro-Fuzzy
...
> **Κανόνας επιλογής:** Αν προτεραιότητα η **ερμηνευσιμότητα** → Mamdani. Αν προτεραιότητα η **ταχύτητα** ή η **εκπαίδευση από δεδομένα** → Sugeno.
