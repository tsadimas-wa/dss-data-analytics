...
**Γιατί χρειαζόμαστε παραγωγίσιμες συναρτήσεις;**

Η εκπαίδευση γίνεται με **Gradient Descent**: το δίκτυο μετράει το σφάλμα του και διορθώνει τα βάρη ακολουθώντας την **κλίση** της συνάρτησης απωλειών — όπως ένας τυφλός ορειβάτης που νιώθει το έδαφος για να κατεβεί στην κοιλάδα. Για να υπάρχει κλίση, η παράγωγος της $f(z)$ **πρέπει να είναι διαφορετική από 0**. *(Αναλύεται διεξοδικά στη διαφάνεια «Πώς Μαθαίνουν τα Δίκτυα»)*
...
Ομαλή καμπύλη σχήματος S, συμπιέζει οποιοδήποτε $z$ στο $(0,1)$ → ιδανική για πιθανότητες. **Πρόβλημα:** για $|z| \gg 0$ η καμπύλη γίνεται επίπεδη → παράγωγος ≈ 0 → η εκπαίδευση «παγώνει». Αυτό ονομάζεται **vanishing gradient** (*εξαφάνιση κλίσης*): το σήμα διόρθωσης που ταξιδεύει προς τα πίσω στο δίκτυο γίνεται τόσο μικρό που τα πρώτα επίπεδα παύουν να μαθαίνουν.
...
Όπως Sigmoid αλλά έξοδος στο $(-1,1)$ — μηδενοκεντρική. Ίδιο vanishing gradient πρόβλημα (η παράγωγος μηδενίζεται για μεγάλα $|z|$).
