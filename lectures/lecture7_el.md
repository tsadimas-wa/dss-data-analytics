...
$$z = w_0 + w_1 x_1 + w_2 x_2 + \ldots + w_n x_n = \mathbf{w}^\top \mathbf{x} + b$$

$$\hat{y} = f(z)$$

* $x_i$: είσοδοι (χαρακτηριστικά)
* $w_i$: βάρη (μαθαίνονται κατά την εκπαίδευση)
* $b$: bias (μετατόπιση)
* $f$: **συνάρτηση ενεργοποίησης**

> **Σύνδεση:** Ο Perceptron με sigmoid είναι ακριβώς η **Logistic Regression** — το απλούστερο νευρωνικό δίκτυο.
...
| **Softmax** | $\frac{e^{z_i}}{\sum e^{z_j}}$ | Πολυκλασσικη έξοδος |
...
**Παράδειγμα — Churn prediction:**

```
Είσοδοι (10 χαρακτηριστικά)
    ↓
Hidden Layer 1 (64 νευρώνες) → ReLU
    ↓
Hidden Layer 2 (32 νευρώνες) → ReLU
    ↓
Έξοδος (1 νευρώνας) → Sigmoid
    ↓
P(Ακύρωση) ∈ [0, 1]
```
