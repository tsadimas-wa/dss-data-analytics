...
<!-- _class: small -->
# Συναρτήσεις Ενεργοποίησης — Τι Είναι και Γιατί Χρειάζονται

Κάθε νευρώνας υπολογίζει $z = \mathbf{w}^\top\mathbf{x} + b$ και στη συνέχεια εφαρμόζει μια **συνάρτηση ενεργοποίησης** $f(z)$.

> Χωρίς $f(z)$, το δίκτυο — όσα επίπεδα κι αν έχει — λύνει μόνο **γραμμικά** προβλήματα. Οι συναρτήσεις ενεργοποίησης δίνουν στα νευρωνικά δίκτυα την ικανότητα να μαθαίνουν **μη-γραμμικά μοτίβα**.
...
Εμπνευσμένη από τη βιολογία: εκπέμπει 1 αν $z>0$, αλλιώς 0.  
**Πρόβλημα:** η παράγωγός της είναι παντού 0 — το Gradient Descent δεν βλέπει κλίση, τα βάρη δεν ενημερώνονται ποτέ.

**🟡 Sigmoid** — Έξοδος binary

Ομαλή καμπύλη σχήματος S, συμπιέζει οποιοδήποτε $z$ στο $(0,1)$ → ιδανική για πιθανότητες. **Πρόβλημα:** για $|z| \gg 0$ η καμπύλη γίνεται επίπεδη → παράγωγος ≈ 0 (*vanishing gradient*) → η εκπαίδευση «παγώνει».

</div>

<div>

**🟢 ReLU** — Κρυφά επίπεδα (default)

$\max(0,z)$: αν $z>0$ το αφήνει αναλλοίωτο, αλλιώς 0. Εξαιρετικά γρήγορη, λύνει το vanishing gradient. **Πρόβλημα:** «**νεκροί νευρώνες**» — αν ένας νευρώνας βγάζει πάντα $z<0$, σταματά να μαθαίνει μόνιμα.

**🔵 Tanh** — Κρυφά επίπεδα (παλαιότερα)

Όπως Sigmoid αλλά έξοδος στο $(-1,1)$ — μηδενοκεντρική. Ίδιο vanishing gradient πρόβλημα.

**🟣 Softmax** — Πολυκλασική έξοδος

Παίρνει διάνυσμα $z$ και βγάζει πιθανότητες που αθροίζουν στο 1.  
...
Για $z = -0.12$ (θα το υπολογίσουμε στο επόμενο παράδειγμα) και άλλες τιμές:
...
> Κρυφά επίπεδα → **ReLU**  
> Έξοδος binary → **Sigmoid**  
> Έξοδος multi-class → **Softmax**  
