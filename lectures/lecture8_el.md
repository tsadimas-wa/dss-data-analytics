...
![Κύκλος Γενετικού Αλγορίθμου](../img/lec8/ga_cycle.png)
...
Πριν ξεκινήσει ο αλγόριθμος, πρέπει να αναπαραστήσουμε μια λύση ως **χρωμόσωμα**.

**Παράδειγμα: Πρόβλημα του Σακιδίου (Knapsack Problem)**
> Έχουμε 5 αντικείμενα με βάρος και αξία. Στόχος: μεγιστοποίηση αξίας χωρίς να ξεπεράσουμε το όριο βάρους.

**Κωδικοποίηση:** Δυαδικό διάνυσμα μήκους 5 — `1` = το αντικείμενο είναι στο σακίδιο, `0` = δεν είναι.

| Λύση (Χρωμόσωμα) | Αντικείμενα που περιέχει |
|---|---|
| `[1, 0, 1, 0, 0]` | 1ο και 3ο |
| `[0, 1, 1, 1, 0]` | 2ο, 3ο και 4ο |

**Παράδειγμα: TSP (Traveling Salesperson Problem)**
> Βρες τη συντομότερη διαδρομή που επισκέπτεται 5 πόλεις (A, B, C, D, E).

**Κωδικοποίηση:** Μετάθεση (permutation) των πόλεων.

| Λύση (Χρωμόσωμα) | Σειρά επίσκεψης |
|---|---|
| `[A, C, B, E, D]` | A → C → B → E → D → A |
| `[B, D, A, C, E]` | B → D → A → C → E → B |

> Η επιλογή της σωστής κωδικοποίησης είναι το **πιο κρίσιμο βήμα** στο σχεδιασμό ενός Γενετικού Αλγορίθμου.
