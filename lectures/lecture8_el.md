...
- **Μέθοδοι (για δυαδική κωδικοποίηση):**
  - **One-Point Crossover:** Επιλέγεται ένα τυχαίο σημείο στο χρωμόσωμα. Το πρώτο παιδί παίρνει το πρώτο μέρος από τον πρώτο γονέα και το δεύτερο μέρος από τον δεύτερο γονέα. Παράδειγμα: `[111|00]` + `[000|11]` → `[111|11]`.
...
**Πρόβλημα:** Μεγιστοποίηση $f(x) = x^2$ για $x \in \{0,\ldots,31\}$ — **5-bit** δυαδικό χρωμόσωμα (Goldberg, 1989).
**Αποκωδικοποίηση:** `01101` $= 0{\times}16 + 1{\times}8 + 1{\times}4 + 0{\times}2 + 1{\times}1 = 13$, άρα $f(13) = 13^2 = 169$.

| Άτομο | Χρωμόσωμα | $x$ | $f(x)=x^2$ | $P(\text{επιλογής})$ | Αναμ. αντίγρ. |
| :---: | :---: | :---: | :---: | :---: | :---: |
| 1 | `01101` | 13 | 169 | 169/1170 = **14,4%** | **0,58** |
| 2 | `11000` | 24 | 576 | 576/1170 = **49,2%** | **1,97** |
| 3 | `01000` | 8 | 64 | 64/1170 = **5,5%** | **0,22** |
| 4 | `10011` | 19 | 361 | 361/1170 = **30,9%** | **1,23** |
| **Σύνολο** | | | **1170** | **100%** | **4,00** |

$$P(x_i) = \frac{f(x_i)}{\displaystyle\sum_j f(x_j)} \qquad \text{Αναμ. αντίγρ.} = P(x_i) \times N$$

**Τι μας λένε τα "Αναμενόμενα Αντίγραφα"** (για πληθυσμό $N=4$):

- **Άτομο 2** ($x{=}24$, $f{=}576$): αναμένεται να επιλεγεί **~2 φορές** — κυριαρχεί στην επόμενη γενιά.
- **Άτομο 3** ($x{=}8$, $f{=}64$): αναμένεται να επιλεγεί **~0 φορές** — πιθανόν να εξαφανιστεί.
- Η μη-γραμμική $f(x){=}x^2$ ενισχύει τις διαφορές: το 2× καλύτερο χρωμόσωμα ($24$ vs $13$) παίρνει **3,4×** περισσότερες πιθανότητες — αυτό λέγεται **πίεση επιλογής (selection pressure)**.
...
**Πρόβλημα:** 4 πόλεις (A,B,C,D) — βρες τη συντομότερη κυκλική διαδρομή.
...
<!-- _class: xxsmall -->
# Παράδειγμα TSP — Επιλογή & Διασταύρωση (Γενιά 0 → 1)

**Αρχικός Πληθυσμός (Γενιά 0):**

| Χρωμόσωμα | Απόσταση | Fitness $f$ | $P$ |
| :--- | :---: | :---: | :---: |
| **`[A,B,C,D]`** | **600 km** | **0,00167** | **37,4%** |
| `[A,B,D,C]` | 685 km | 0,00146 | 32,7% |
| `[A,C,B,D]` | 745 km | 0,00134 | 30,0% |

**Επιλογή (Ρουλέτα):** P1 = `[A,B,C,D]` · P2 = `[A,B,D,C]`

**Διασταύρωση OX** — αντιγράφουμε θέσεις **0–1** από P1, συμπληρώνουμε από P2 (παρ. διπλότυπα):
...
<!-- _class: xxsmall -->
# Παράδειγμα TSP — Μετάλλαξη & Αποτέλεσμα

**4. Μετάλλαξη (Mutation - Swap):**
Το Παιδί 1 (`[A,B,D,C]`) υφίσταται τυχαία ανταλλαγή θέσεων 2↔3:

- Πριν: `[A, B, D, C]` = 685 km
- Μετά: `[A, B, C, D]` = **600 km** ✓

**5. Αντικατάσταση (Replacement - με Ελιτισμό):**
Διατηρούμε το καλύτερο άτομο από την Γενιά 0 (Ελιτισμός) και αντικαθιστούμε τα υπόλοιπα με τα νέα παιδιά.
...
> **Αποτέλεσμα (Γενιά 1):** Ο πληθυσμός έχει συγκλίνει στη βέλτιστη διαδρομή `A→B→C→D→A = 600 km`. Η διαδικασία επαναλαμβάνεται για N γενιές μέχρι να ικανοποιηθεί κριτήριο τερματισμού.
