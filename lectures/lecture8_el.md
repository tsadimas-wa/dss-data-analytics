...
<!-- _class: xxsmall -->
# 4. Προσομοιωμένη Ανόπτηση (SA) — Έμπνευση & Αλγόριθμος

**Έμπνευση:** Η διαδικασία ανόπτησης (annealing) στη μεταλλουργία. Αυτή περιλαμβάνει τη θέρμανση ενός μετάλλου σε υψηλή θερμοκρασία, επιτρέποντας στα άτομά του να κινούνται ελεύθερα και να ξεφεύγουν από τοπικές διατάξεις. Στη συνέχεια, το μέταλλο ψύχεται *πολύ αργά* και σταδιακά. Αυτή η αργή ψύξη επιτρέπει στα άτομα να τακτοποιηθούν σε μια δομή ελάχιστης ενέργειας (τέλειος κρύσταλλος), αποφεύγοντας ελαττώματα και παγίδευση σε τοπικά βέλτιστα.
...
**Πώς Λειτουργεί ο Αλγόριθμος:**
1.  **Αρχικοποίηση:**
    *   Ξεκινάμε με μια τυχαία αρχική λύση $S$ (π.χ. μια τυχαία διαδρομή στο TSP).
    *   Ορίζουμε μια υψηλή αρχική θερμοκρασία $T$ (π.χ. $T=100$).
    *   Καθορίζουμε ένα **σχήμα ψύξης (cooling schedule)**, δηλαδή πώς θα μειώνεται η θερμοκρασία σε κάθε βήμα.
2.  **Επανάληψη (για κάθε βήμα ψύξης):**
    *   Για έναν συγκεκριμένο αριθμό επαναλήψεων σε κάθε θερμοκρασία:
        a. Δημιουργούμε μια «γειτονική» λύση $S_{new}$ από την τρέχουσα λύση $S$ (π.χ., στο TSP, αλλάζουμε τη σειρά δύο πόλεων).
        b. Υπολογίζουμε τη διαφορά στο κόστος (ή ενέργεια): $\Delta E = Cost(S_{new}) - Cost(S)$.
        c. **Απόφαση Αποδοχής (Metropolis Criterion):**
            - Αν $\Delta E < 0$ (η νέα λύση είναι **καλύτερη**), την αποδεχόμαστε πάντα: $S \leftarrow S_{new}$.
            - Αν $\Delta E \ge 0$ (η νέα λύση είναι **χειρότερη**), την αποδεχόμαστε με πιθανότητα $P = e^{-\Delta E / T}$.
            *   Όσο μεγαλύτερο το $\Delta E$, τόσο μικρότερη η πιθανότητα αποδοχής.
            *   Όσο υψηλότερη η $T$, τόσο μεγαλύτερη η πιθανότητα — επιτρέπει **εξερεύνηση** στην αρχή.
