...
**Πρόβλημα:** Μεγιστοποίηση του $f(x) = x$ για ακέραιο $x \in \{0,\ldots,15\}$.
**Κωδικοποίηση:** Κάθε χρωμόσωμα είναι μια **4-bit δυαδική** αναπαράσταση του $x$.
**Αποκωδικοποίηση (παράδειγμα):** `1000` $= 1{\times}2^3 + 0{\times}2^2 + 0{\times}2^1 + 0{\times}2^0 = 8$, άρα $f(x) = 8$.

**Πληθυσμός 4 χρωμοσωμάτων:**

| Άτομο | Χρωμόσωμα | Αποκωδ. $x$ | Fitness $f(x){=}x$ | $P(\text{επιλογής})$ |
| :---: | :---: | :---: | :---: | :---: |
| Α | `1000` | 8 | **8** | 8/25 = **0,32** |
| Β | `0111` | 7 | **7** | 7/25 = **0,28** |
| Γ | `0110` | 6 | **6** | 6/25 = **0,24** |
| Δ | `0100` | 4 | **4** | 4/25 = **0,16** |
| **Σύνολο** | | | **25** | **1,00** |

$$P(x_i) = \frac{f(x_i)}{\displaystyle\sum_j f(x_j)}$$

Το **Α** (`1000` = 8) έχει τη μεγαλύτερη τιμή → καταλαμβάνει το **32%** του τροχού και επιλέγεται συχνότερα. Ακόμα και το **Δ** (`0100` = 4, η χειρότερη λύση) έχει **16%** πιθανότητα — αυτό διατηρεί **γενετική ποικιλομορφία** και αποτρέπει την πρόωρη σύγκλιση σε τοπικό βέλτιστο.
