...
chance_node  (ax, *nodes['disease'], 'Ασθένεια',        'P(Α) = 0.30')
chance_node  (ax, *nodes['test'],    'Αποτέλεσμα\nΤεστ','P(T|Α)')
decision_node(ax, *nodes['treat'],   'Θεραπεία;',       'Ναι / Όχι / Χειρ.')
utility_node (ax, *nodes['utility'], 'Χρησιμότητα',     'EU = Σ P·U')
...
ax.annotate("", xy=(11.5, 2.80), xytext=(11.5, 3.50 - 0.70),
            arrowprops=dict(arrowstyle='-', color='#78909C', lw=1.8), zorder=2)
ax.text(6.75, 2.55, 'άμεση επίδραση ασθένειας στη χρησιμότητα',
...
ax.annotate("", xy=(8.4, 4.10), xytext=(8.4, 3.5+0.55),
            arrowprops=dict(arrowstyle='-', color='#B0BEC5', lw=1.5), zorder=2)
ax.text(5.2, 4.35, 'πληροφορία διαθέσιμη κατά την απόφαση',
        ha='center', fontsize=8.5, color='#78909C', style='italic')
...
# ══════════════════════════════════════════════════════════════════════════════
tbl_x, tbl_y = 6.8, 2.2
ax.text(tbl_x, tbl_y, 'Αναμενόμενη Χρησιμότητα (EU) ανά επιλογή:',
        ha='center', fontsize=9.5, fontweight='bold', color='#37474F')

rows = [
    ('Θεραπεία Α',  'EU = 0.30×80 + 0.70×(-10) = +17'),
    ('Θεραπεία Β',  'EU = 0.30×60 + 0.70×(+5)  = +21.5  ✓'),
    ('Χωρίς θεραπεία', 'EU = 0.30×(-50)+ 0.70×10  = −8'),
]
...
# ══════════════════════════════════════════════════════════════════════════════
legend_items = [
    mpatches.Patch(color=C_CHANCE,   label='Τυχαίος κόμβος — chance (ελλειψοειδής)'),
    mpatches.Patch(color=C_DECISION, label='Κόμβος απόφασης — decision (τετράγωνος)'),
    mpatches.Patch(color=C_UTILITY,  label='Κόμβος χρησιμότητας — utility (ρόμβος)'),
]
ax.legend(handles=legend_items, loc='upper left', fontsize=9,
          framealpha=0.9, edgecolor='#ccc', ncol=3,
          bbox_to_anchor=(0.0, 1.0))

ax.set_title('Δίκτυο Αποφάσεων (Influence Diagram) — Ιατρική Θεραπεία',
             fontsize=13, fontweight='bold', pad=28)

fig.tight_layout()
out = 'lectures_material/influence_diagram.png'
