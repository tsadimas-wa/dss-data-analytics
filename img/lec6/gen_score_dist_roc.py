"""Generates score_dist_roc.png — score distributions with TP/TN/FP/FN bars + ROC."""
import numpy as np
import matplotlib
...
n0, n1 = 300, 200
scores0 = np.clip(norm.rvs(loc=0.30, scale=0.13, size=n0), 0.01, 0.99)  # Παραμονή
scores1 = np.clip(norm.rvs(loc=0.65, scale=0.14, size=n1), 0.01, 0.99)  # Ακύρωση
...
def draw_panel(ax, t, tc):
    ...
    ymax = 58
    ax.set_ylim(0, ymax)
    ax.text(t/2,      ymax*0.92, '← Πρόβλεψη: Παραμονή',
            ha='center', fontsize=7.5, color='#333', style='italic')
    ax.text((t+1)/2,  ymax*0.92, 'Πρόβλεψη: Ακύρωση →',
            ha='center', fontsize=7.5, color='#333', style='italic')
    ...
    ax.set_xlabel('P(Ακύρωση)', fontsize=8.5)
    ax.set_ylabel('Αριθμός δειγμάτων', fontsize=8)
    ax.set_xlim(0, 1)
...
fig.suptitle('Από τις Κατανομές Σκορ στην Καμπύλη ROC', fontsize=13,
             fontweight='bold')
...
legend_patches = [
    Patch(color=C_TN, label='TN — Παραμονή, προβλέφθηκε σωστά'),
    Patch(color=C_FN, label='FN — Ακύρωση, ΔΕΝ εντοπίστηκε (χάσαμε)'),
    Patch(color=C_TP, label='TP — Ακύρωση, εντοπίστηκε σωστά'),
    Patch(color=C_FP, label='FP — Παραμονή, σημαιοδοτήθηκε λανθασμένα'),
]
...
ax_roc.plot([0,1],[0,1], color='#999', lw=1.2, linestyle=':', label='Τυχαίο')
...
ax_roc.set_xlabel('FPR (Ψεύτικοι Συναγερμοί)', fontsize=9)
ax_roc.set_ylabel('TPR = Recall', fontsize=9)
ax_roc.set_title('Καμπύλη ROC\nΚάθε σημείο = ένα threshold', fontsize=10, fontweight='bold')
...
plt.tight_layout()
out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/score_dist_roc.png'
