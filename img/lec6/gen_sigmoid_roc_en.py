"""Generates sigmoid_roc_en.png — sigmoid with thresholds (left) + resulting ROC points (right)."""
import numpy as np
import matplotlib
...
ax1.plot(z, sig, color='#333333', lw=2.5, label='Sigmoid $P(\\text{Churn})$')
...
ax1.set_title('Sigmoid — Different Thresholds', fontsize=11, fontweight='bold')
ax1.legend(fontsize=9, framealpha=0.95, loc='upper left')
ax1.set_ylim(-0.05, 1.08)
ax1.grid(alpha=0.22)

# Annotations
ax1.annotate('Low t → more samples\nclassified as Churn\n↑ Recall, ↓ Precision',
             xy=(-4, 0.2), xytext=(-5.8, 0.55),
             arrowprops=dict(arrowstyle='->', color='#6A1B9A', lw=1.1),
             fontsize=8, color='#6A1B9A')
ax1.annotate('High t → fewer samples\nclassified as Churn\n↑ Precision, ↓ Recall',
             xy=(3.5, 0.8), xytext=(1.2, 0.97),
             arrowprops=dict(arrowstyle='->', color='#C62828', lw=1.1),
             fontsize=8, color='#C62828')

# ── Panel 2: ROC curve + highlighted points ───────────────────────────────────
ax2.plot(fpr_all, tpr_all, color='#1A237E', lw=2.2, label='ROC Curve', zorder=2)
ax2.plot([0, 1], [0, 1], color='#999', lw=1.3, linestyle=':', label='Random model')
...
ax2.set_title('ROC Curve — Each Point = One Threshold', fontsize=11, fontweight='bold')
...
plt.tight_layout()
out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/sigmoid_roc_en.png'
