"""Generates kmeans_illustration_en.png — 4-step convergence + elbow method."""
import numpy as np
import matplotlib
...
labels_cls = ['Occasional', 'Regular', 'VIP']
n_per = 25
...
steps.append((centroids.copy(), None, 'Step 1: Initial Random Centers'))
for i in range(1, 4):
    labels_iter = assign(X, centroids)
    steps.append((centroids.copy(), labels_iter.copy(),
                  f'Step {i+1}: Assignment & New Centers'))
    centroids = update(X, labels_iter, K)

# Final step
labels_final = assign(X, centroids)
steps.append((centroids.copy(), labels_final.copy(), 'Convergence'))
...
ax_final.set_title('Final Clustering (K=3) — Customer Segmentation',
                   fontsize=10, fontweight='bold')
ax_final.legend(fontsize=9, framealpha=0.9)
ax_final.grid(alpha=0.2)
ax_final.set_xlabel('Feature 1', fontsize=9)
ax_final.set_ylabel('Feature 2', fontsize=9)
...
ax_elbow.axvline(3, color='#C62828', lw=1.8, linestyle='--', label='K=3 (elbow)')
ax_elbow.scatter([3], [inertias[2]], color='#C62828', s=120, zorder=5)
ax_elbow.annotate('Elbow\n(K=3)', xy=(3, inertias[2]), xytext=(4.2, inertias[2] + 15),
                  arrowprops=dict(arrowstyle='->', color='#C62828', lw=1.2),
                  fontsize=9, color='#C62828')
ax_elbow.set_xlabel('Number of Clusters K', fontsize=10)
ax_elbow.set_ylabel('Inertia (WCSS)', fontsize=10)
ax_elbow.set_title('Elbow Method — Selecting K', fontsize=10, fontweight='bold')
ax_elbow.legend(fontsize=9, framealpha=0.9)
ax_elbow.grid(alpha=0.25)

plt.tight_layout()
out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/kmeans_illustration_en.png'
