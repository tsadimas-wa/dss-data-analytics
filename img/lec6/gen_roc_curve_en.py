"""Generates roc_curve_en.png — ROC curve illustration with multiple models."""
import numpy as np
import matplotlib
...
ax.plot([0, 1], [0, 1], color='#999999', lw=1.4, linestyle=':', label='Random model (AUC = 0.50)')
...
ax.annotate('Ideal model\n(FPR=0, TPR=1)',
            xy=(0, 1), xytext=(0.12, 0.85),
            arrowprops=dict(arrowstyle='->', color='#333', lw=1.3),
            fontsize=9, color='#333')

ax.set_xlabel('False Positive Rate (FPR)\n← Fewer false alarms', fontsize=10)
ax.set_ylabel('True Positive Rate (TPR = Recall)\n← More correct detections', fontsize=10)
ax.set_title('ROC Curve — Decision Tree', fontsize=12, fontweight='bold')
...
ax.text(0.23, 0.50, 'threshold ↑\n(stricter)', fontsize=8, color='#555')
ax.annotate('', xy=(0.28, 0.60), xytext=(0.12, 0.77),
            arrowprops=dict(arrowstyle='->', color='#555', lw=1.2))
ax.text(0.04, 0.79, 'threshold ↓\n(more lenient)', fontsize=8, color='#555')

plt.tight_layout()
out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/roc_curve_en.png'
