"""Generates knn_illustration.png — two-panel k-NN figure (k=1 vs k=5 + decision boundary)."""
import numpy as np
import matplotlib
...
# ── Shared data ───────────────────────────────────────────────────────────────
n = 20
X0 = np.random.randn(n, 2) * 0.9 + np.array([-1.5,  1.5])  # Παραμονή (blue)
X1 = np.random.randn(n, 2) * 0.9 + np.array([ 1.5, -1.5])  # Ακύρωση (red)
new_point = np.array([0.2, 0.3])  # query point

# ── Panel 1: k=1 vs k=5 voting ───────────────────────────────────────────────
for ax, k, title in [(ax1, 5, 'k-NN  (k=5) — Ψηφοφορία Πλειοψηφίας')]:
    ax.scatter(X0[:, 0], X0[:, 1], color='#1565C0', s=60, label='Παραμονή',
               alpha=0.8, zorder=3)
    ax.scatter(X1[:, 0], X1[:, 1], color='#C62828', s=60, label='Ακύρωση',
               alpha=0.8, zorder=3)

    # Query point
    ax.scatter(*new_point, color='#F9A825', s=200, marker='*',
               edgecolors='black', linewidths=1.2, zorder=6, label='Νέο δείγμα')
    ...
    # Vote annotation
    result = 'Παραμονή' if votes[0] > votes[1] else 'Ακύρωση'
    res_color = '#1565C0' if votes[0] > votes[1] else '#C62828'
    ax.annotate(
        f'k={k}: {votes[0]}× Παραμονή, {votes[1]}× Ακύρωση\n→ {result}',
    ...
    ax.set_xlabel('Χαρακτηριστικό 1', fontsize=9)
    ax.set_ylabel('Χαρακτηριστικό 2', fontsize=9)
    ax.grid(alpha=0.2)
...
for k_val, ax, title in [(1, ax1, 'k=1 — Overfitting'), (9, ax2, 'k=9 — Ομαλότερο Όριο')]:
    ...
    ax.scatter(X0[:, 0], X0[:, 1], color='#1565C0', s=50, label='Παραμονή',
               alpha=0.85, zorder=3)
    ax.scatter(X1[:, 0], X1[:, 1], color='#C62828', s=50, label='Ακύρωση',
               alpha=0.85, zorder=3)
...
plt.tight_layout()
out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/knn_illustration.png'
