"""Generates svm_illustration_en.png — two-panel SVM figure (linear + kernel trick)."""
import numpy as np
import matplotlib
...
ax1.scatter(X1[:, 0], X1[:, 1], color='#1565C0', s=50, label='Stay', zorder=3, alpha=0.8)
ax1.scatter(X2[:, 0], X2[:, 1], color='#C62828', s=50, label='Churn', zorder=3, alpha=0.8)
...
ax1.set_ylim(-5, 5)
ax1.set_title('Linear SVM — Margin Maximization', fontsize=11, fontweight='bold')
...
ax2.scatter(X_inner[:, 0], X_inner[:, 1], color='#1565C0', s=50, label='Stay', zorder=3, alpha=0.8)
ax2.scatter(X_outer[:, 0], X_outer[:, 1], color='#C62828', s=50, label='Churn', zorder=3, alpha=0.8)
...
ax2.set_title('Kernel Trick (RBF) — Non-Linear Separation', fontsize=11, fontweight='bold')
...
plt.tight_layout()
out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/svm_illustration_en.png'
