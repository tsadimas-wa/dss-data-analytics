"""Generates decision_tree_en.png with fully English labels and Gini values per node."""
import matplotlib
...
# ── nodes ────────────────────────────────────────────────────────────────────
node(ax, ROOT_x, Y['root'], 'Tenure < 12 months?\n(Root)', ROOT,
     gini=0.48, width=3.2, height=0.78)

node(ax, L1a_x, Y['l1'], 'Monthly Charges\n> 70€?', INNER, gini=0.32)
node(ax, L1b_x, Y['l1'], 'Contract =\nMonth-to-Month?', INNER, gini=0.23)

node(ax, L2a_x, Y['l2'], '✗ CHURN\nnew + expensive plan', CANCEL, gini=0.00)
node(ax, L2b_x, Y['l2'], 'Contract =\nMonth-to-Month?',   INNER,  gini=0.45)
node(ax, L2c_x, Y['l2'], 'Monthly Charges\n> 85€?',       INNER,  gini=0.38)
node(ax, L2d_x, Y['l2'], '✓ STAY\ncommitted',             STAY,   gini=0.00)

node(ax, L3a_x, Y['l3'], '✗ CHURN\nno commitment',            CANCEL, gini=0.00)
node(ax, L3b_x, Y['l3'], '✓ STAY\nannual contract',           STAY,   gini=0.00)
node(ax, L3c_x, Y['l3'], '✗ CHURN\nexpensive + no commitment', CANCEL, gini=0.00)
node(ax, L3d_x, Y['l3'], '✓ STAY\nmoderate charges',          STAY,   gini=0.00)
...
# ── edge labels ──────────────────────────────────────────────────────────────
edge_label(ax, (ROOT_x + L1a_x)/2 - 0.2, 6.50, 'YES — new customer')
edge_label(ax, (ROOT_x + L1b_x)/2 + 0.2, 6.50, 'NO — existing customer')
edge_label(ax, (L1a_x + L2a_x)/2 - 0.1, 4.68, 'YES')
edge_label(ax, (L1a_x + L2b_x)/2 + 0.1, 4.68, 'NO')
edge_label(ax, (L1b_x + L2c_x)/2 - 0.1, 4.68, 'YES')
edge_label(ax, (L1b_x + L2d_x)/2 + 0.1, 4.68, 'NO')
edge_label(ax, (L2b_x + L3a_x)/2 - 0.1, 2.86, 'YES')
edge_label(ax, (L2b_x + L3b_x)/2 + 0.1, 2.86, 'NO')
edge_label(ax, (L2c_x + L3c_x)/2 - 0.1, 2.86, 'YES')
edge_label(ax, (L2c_x + L3d_x)/2 + 0.1, 2.86, 'NO')

out = '/home/rg/Teaching/uniwa/dss-data-analytics/img/lec6/decision_tree_en.png'
plt.tight_layout()
