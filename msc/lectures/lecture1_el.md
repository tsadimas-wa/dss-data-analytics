...
|---|---|---|
| Έναρξη / Λήξη | Ελλειψοειδές | Αρχή ή τέλος διεργασίας |
| Δραστηριότητα | Ορθογώνιο | Εκτέλεση εργασίας |
| Απόφαση | Ρόμβος | Διακλάδωση (Ναι/Όχι) |
| Ροή | Βέλος | Σειρά εκτέλεσης |
| Έγγραφο | Ορθογώνιο με κυματοειδή βάση | Παραγόμενο ή χρησιμοποιούμενο έγγραφο |

**Παράδειγμα — Διεργασία Παραγγελίας:**

```
[Παραγγελία] → [Έλεγχος Αποθέματος] → ◇ Υπάρχει; →Ναι→ [Αποστολή] → [Τιμολόγηση] → [Τέλος]
                                              ↓Όχι
                                        [Παραγγελία από Προμηθευτή] → [Παραλαβή] → [Αποστολή]
```

> **Εργαλεία:** MS Visio, Lucidchart, draw.io, ARIS — *Πηγή: Harrington (1991), Business Process Improvement*
...
**Παράδειγμα:** Toyota (mass customization) — κατάφερε να συνδυάσει Assembly Line με Ευελιξία μέσω TPS.
...
**Ορισμός:** Μικρές διακυμάνσεις στη ζήτηση καταναλωτή **ενισχύονται** όσο ανεβαίνουμε στην αλυσίδα → υπερβολικές παραγγελίες/αποθέματα στους προμηθευτές *(Lee et al., 1997)*.

```
Πελάτης:     ~~~ (μικρή διακύμανση)
Λιανέμπορος: ~~~~~ (μεγαλύτερη)
Χονδρέμπορος: ~~~~~~~~~
Παραγωγός:   ~~~~~~~~~~~~~~~~~ (τεράστια διακύμανση)
```

**Αιτίες:**
- Batch ordering (παραγγελία ανά εβδομάδα αντί καθημερινά)
- Κερδοσκοπική αγορά λόγω εκπτώσεων
- Ανακριβείς προβλέψεις — ο καθένας προσθέτει δικό του «buffer»
- Έλλειψη διαμοιρασμού πληροφοριών

**Αντιμετώπιση:** VMI (Vendor Managed Inventory), EDI, κοινές προβλέψεις (CPFR), JIT.

> *Πηγή: Lee, H., Padmanabhan, V. & Whang, S. (1997). "The Bullwhip Effect in Supply Chains". Sloan Management Review.*
...
**Εξέλιξη:**
- MRP → MRP II (προσθήκη capacity planning)
- MRP II → **ERP** (ενοποίηση όλων των λειτουργιών)
...
Ολοκληρωμένο πληροφοριακό σύστημα που ενοποιεί:
...
**Κυρίαρχα συστήματα:** SAP, Oracle, Microsoft Dynamics

</div>
</div>

> *Πηγή: Vollmann, Berry, Whybark & Jacobs (2005). Manufacturing Planning and Control Systems. McGraw-Hill.*
...
# 8. Μετρήσεις Απόδοσης Λειτουργιών (KPIs)

<!-- _class: small -->

**Τι μετράμε για να ελέγξουμε τις λειτουργίες;**
...
| **Capacity Utilization** | Πραγματική παραγωγή / Μέγιστη |
