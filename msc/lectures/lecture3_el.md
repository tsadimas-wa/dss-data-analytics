...
6.  **Εργαστηριακός Οδηγός**
    -   **Signavio:** Μετρικές Απόδοσης, Μοντελοποίηση, ανάλυση κόστους & προσομοίωση
    -   **Camunda.io:** Σχεδίαση BPMN & εκτέλεση online
...
<!--
Τα Gateways αποτελούν κρίσιμα σημεία απόφασης στη ροή. Το πιο συχνό λάθος είναι η σύγχυση μεταξύ XOR και OR. Ιδιαίτερη προσοχή χρειάζεται στο Event-Based Gateway: εδώ η απόφαση δεν λαμβάνεται βάσει δεδομένων, αλλά περιμένουμε να δούμε ποιο εξωτερικό γεγονός θα συμβεί πρώτο, εισάγοντας έτσι ασύγχρονη λογική. Θυμηθείτε επίσης τον κανόνα ονοματολογίας: τα Gateways που παίρνουν απόφαση (Split) πρέπει να έχουν όνομα σε μορφή ερώτησης με απαντήσεις στα βελάκια, ενώ τα Gateways που απλώς ενώνουν μονοπάτια (Join) μένουν ανώνυμα.
-->
---
...
<!--
Το AND Gateway κλωνοποιεί τα tokens. Προσοχή όμως: ένα AND Split πρέπει ΠΑΝΤΑ να συναντά ένα AND Join αργότερα. Αλλιώς, τα διπλά tokens θα συνεχίσουν να τρέχουν ανεξέλεγκτα, εκτελώντας τα επόμενα Tasks πολλαπλές φορές και προκαλώντας χάος στο σύστημα. Προσοχή επίσης στο σφάλμα "Deadlock": αν ένα από τα δύο παράλληλα μονοπάτια τερματίσει πρόωρα πριν φτάσει στο AND-Join, το άλλο token θα περιμένει εκεί αιώνια! Για να το αποφύγετε, αφήστε τους κλάδους να τερματίσουν ανεξάρτητα χωρίς AND-Join.
-->
---
...
**Βήμα 3: Reporting → Process Cost Analysis**
- Από τον Explorer: επιλέξτε αρχείο → Reporting → Process Cost Analysis → Run
- Το Signavio εμφανίζει: Συνολικό Ετήσιο Κόστος διεργασίας + Μέσος Cycle Time

**Βήμα 4: Προσομοίωση (Simulation)**
- Μέσα από τον Editor: **Edit → Simulate BPMN diagram**
- Τρέξτε σενάριο (π.χ. Cases: 5000) για να βγει στην επιφάνεια το bottleneck (συνήθως ο Task «Έλεγχος HR» λόγω Wait Time)
...
# 6.3 Εκτελέσιμο BPMN στην Πράξη: Camunda.io
...
- **Tasklist** — χρήστης αναλαμβάνει και ολοκληρώνει User Tasks
