...
1.  **Ανασκόπηση από Ενότητα 2**
    ...
    -   **4 Προοπτικές Μοντελοποίησης:** Behavioural, Functional, Informational, Organisational
...
2.  **BPMN 2.0 — Εμβάθυνση**
    ...
    -   **Δεδομένα:** Data Objects, Data Stores, State of Data Objects
    -   **Message Flow & Pools/Lanes:** Επικοινωνία μεταξύ οργανισμών, Message Start Event

</div>
<div>

3.  **CMMN — Case Management Model and Notation**
    -   Φάσμα Διεργασιών: από Δομημένες ως Αδόμητες
    -   Γιατί το BPMN δεν αρκεί πάντα;
    -   **Διεργασίες Εντατικής Γνώσης (KIPs):** Ορισμός & 8 Χαρακτηριστικά
    -   **Στοιχεία CMMN** & Χαρτογράφηση KIPs–CMMN
...
> ⚠️ **Σημείωση συμβόλων:** Τα ✕ / + / ∪ / ⬡ αποτελούν κειμενική απεικόνιση των εικονιδίων gateway. Στο BPMN 2.0 ο ρόμβος με **κύκλο** (Ο) συμβολίζει το OR (Inclusive), όχι το γραμματόσημο ∪.
...
**Κατάσταση Data Object (State)**
Ένα Data Object μπορεί να έχει **κατάσταση** — αυτή ορίζεται μέσα σε αγκύλες `[ ]` δίπλα στο όνομά του.
...
- Η κατάσταση βοηθά να δούμε πώς **εξελίσσεται** ένα έγγραφο κατά τη ροή.

</div>
</div>

<!--
Τα δεδομένα είναι το "αίμα" της διεργασίας. Η κατάσταση Data Object (State) μας δείχνει πού βρίσκεται ένα έγγραφο στον κύκλο ζωής του. Για παράδειγμα η Παραγγελία αρχίζει ως [εκκρεμής], γίνεται [επιβεβαιωμένη] μετά τον έλεγχο, και τελικά [παραδοθείσα] στο τέλος.
-->
---
...
# 3.1 Φάσμα Επιχειρησιακών Διεργασιών
...
# 3.2 Γιατί το BPMN δεν Αρκεί Πάντα;
...
# 3.3 Διεργασίες Εντατικής Γνώσης (KIPs)

**Ορισμός:** Επιχειρησιακές διεργασίες των οποίων η εκτέλεση εξαρτάται κυρίως από τη **γνώση των ανθρώπινων συμμετεχόντων** (knowledge workers). Δεν μπορούν να προκαθοριστεί εξαντλητικά, καθώς κάθε περίπτωση είναι μοναδική.

| Χαρακτηριστικό | Περιγραφή |
|---|---|
| **Γνωσιακή** *(Knowledge-driven)* | Απαιτεί εξειδικευμένη γνώση πεδίου (π.χ. ιατρική, νομική) |
| **Συνεργατική** *(Collaboration-oriented)* | Εμπλέκει πολλούς ειδικούς που συνεργάζονται |
| **Απρόβλεπτη** *(Unpredictable)* | Το μονοπάτι εκτέλεσης δεν μπορεί να καθοριστεί εκ των προτέρων |
| **Αναδυόμενη** *(Emergent)* | Νέες καταστάσεις και ανάγκες ανακαλύπτονται κατά την εκτέλεση |
| **Στόχο-κεντρική** *(Goal-oriented)* | Κατευθύνεται από υψηλόβαθμους στόχους, όχι από σκληρή ροή βημάτων |
| **Συμβάν-κεντρική** *(Event-driven)* | Ενεργοποιείται και κατευθύνεται από γεγονότα |
| **Δεσμός-κεντρική** *(Constraint-driven)* | Διέπεται από κανόνες, νομικές υποχρεώσεις και περιορισμούς |
| **Μη επαναλαμβανόμενη** *(Non-repeatable)* | Κάθε περίπτωση (case) είναι μοναδική — δεν υπάρχει "τυπική" εκτέλεση |

> **Παραδείγματα KIPs:** Διαχείριση ιατρικής υπόθεσης · Νομική αντιδικία · Έρευνα ασφαλιστικής απάτης · IT service management

<!--
Τα KIPs είναι οι διεργασίες που "σπάνε" τα εργαλεία τύπου BPMN. Δεν μπορείς να προκαθορίσεις τι θα κάνεις γιατί εξαρτάται από τι ανακαλύπτεις κατά την πορεία. Ο γιατρός δεν ξέρει εκ των προτέρων ποιες εξετάσεις θα χρειαστούν — το αποφασίζει βάσει των συμπτωμάτων που βλέπει.
-->
---

<!-- _class: small -->

# 3.4 Βασικά Στοιχεία CMMN
...
# 3.5 Χαρτογράφηση KIPs — CMMN: Πώς το CMMN Καλύπτει τα KIPs

Κάθε χαρακτηριστικό των KIPs αντιστοιχεί σε ένα ή περισσότερα **στοιχεία CMMN**:

| Χαρακτηριστικό KIP | Πώς το καλύπτει το CMMN |
|---|---|
| **Γνωσιακή** | Discretionary Items — ο knowledge worker επιλέγει ποια Tasks εκτελεστούν |
| **Συνεργατική** | Human Tasks με ανάθεση σε ρόλους/teams μέσα στο Case |
...
| **Στόχο-κεντρική** | Milestones ορίζουν στόχους — όχι ακολουθίες |
| **Συμβάν-κεντρική** | Sentries αντιδρούν σε γεγονότα (Entry/Exit Criteria) |
| **Δεσμός-κεντρική** | Κανόνες εκφράζονται ως Sentries και Case File constraints |
...
- Προσθέστε τα στοιχεία: Start Event → Task «Υποβολή Αίτησης» → XOR Gateway → (Εγκρίνεται / Απορρίπτεται) → End Events

**Βήμα 2: Ορισμός Παραμέτρων (Process Properties πάνελ)**

| Στοιχείο BPMN | Παράμετρος | Τιμή | Σχόλιο |
|---|---|---|---|
| Διεργασία | Frequency | 200/year | Πόσες εκκινήσεις ετησίως |
| Task «Υποβολή» | Execution Time | 15 min | Χρόνος συμπλήρωσης αίτησης |
| Task «Υποβολή» | Cost | 25 € | Κόστος εργασίας |
...
**Βήμα 3: Analysis → Process Cost Analysis**
- Πάρτε: Analysis → Process Cost Analysis → Run
- Το Signavio εμφανίζει: Συνολικό Ετήσιο Κόστος διεργασίας + Μέσος Cycle Time

**Βήμα 4: Analysis → Simulation**
- Τρέξτε σενάριο 200 εκτελ. για να βγει στην επιφάνεια το bottleneck (συνήθως ο Task «Έλεγχος HR» λόγω Wait Time)
