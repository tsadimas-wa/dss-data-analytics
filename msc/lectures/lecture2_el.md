...
<!-- Το Capacity Planning είναι ίσως η πιο ακριβή απόφαση που καλείται να πάρει μια επιχείρηση: 'πόσο μεγάλο εργοστάσιο χρειαζόμαστε;', 'πόσο μεγάλο στόλο φορτηγών;'. Μια λάθος απόφαση εδώ μπορεί να δεσμεύσει τεράστια κεφάλαια για χρόνια ή να μας αφήσει εκτεθειμένους στον ανταγωνισμό χωρίς να μπορούμε να καλύψουμε τη ζήτηση. -->
...
<!-- Η διαχείριση δυναμικότητας δεν είναι μονοδιάστατη. Ανάλογα με το τι θέλουμε να μετρήσουμε, εστιάζουμε είτε στους ανθρώπους (Resource), είτε στα έργα (Project), είτε στην παραγωγή (Product). Κάθε τύπος απαιτεί διαφορετικά εργαλεία και μετρήσεις. -->
...
<!-- Για να σχεδιάσουμε τη δυναμικότητα, δεν μαντεύουμε. Χρησιμοποιούμε τεχνικές όπως η ανάλυση 'What-if' για να δούμε τις επιπτώσεις πιθανών αλλαγών, ή το Resource Leveling για να μοιράσουμε σωστά τη δουλειά και να μην 'καίμε' τους πόρους μας. Αυτά τα εργαλεία μας βοηθούν να πάρουμε πιο τεκμηριωμένες αποφάσεις. -->
...
<!-- Αφού προβλέψουμε τη ζήτηση, πρέπει να αποφασίσουμε πότε θα επενδύσουμε στη νέα δυναμικότητα. Έχουμε 3 στρατηγικές: Η 'Lead' είναι επιθετική — χτίζουμε το εργοστάσιο πριν έρθει η ζήτηση, με ρίσκο να μείνει άδειο. Η 'Lag' είναι αμυντική — περιμένουμε να δούμε τη ζήτηση και μετά χτίζουμε, με ρίσκο να χάσουμε πωλήσεις. Η 'Match' προσπαθεί να ισορροπήσει, προσθέτοντας δυναμικότητα σταδιακά. Κάθε μία έχει τα υπέρ και τα κατά της. -->
...
# 9.3.3 Case Study: KPIs σε Kanban Board

<!-- _class: casestudy xxsmall -->

Στην ανάπτυξη λογισμικού, τα **Kanban boards** (π.χ. σε Jira, Trello) είναι το οπτικό εργαλείο για τη διαχείριση της ροής εργασίας. Κάθε "ticket" (μια εργασία) κινείται από στήλη σε στήλη, επιτρέποντάς μας να μετράμε κρίσιμα KPIs.

**Παράδειγμα Kanban Board (Τέλος Εβδομάδας):**

| To Do (Backlog) | In Progress (WIP=2) | In Review (WIP=1) | Done (This Week) |
|---|---|---|---|
| Ticket-105 | Ticket-103 (ξεκίνησε Δευτέρα) | Ticket-102 (σε review από Πέμπτη) | Ticket-101 (Τρίτη→Πέμπτη) |
| Ticket-106 | Ticket-104 (ξεκίνησε Τετάρτη) | | Ticket-100 (Δευτέρα→Τετάρτη) |

**KPIs που προκύπτουν:**

| KPI | Υπολογισμός από το Board | Ερμηνεία |
|---|---|---|
| **Throughput** | **2 tickets/εβδομάδα** (Ticket-100, Ticket-101) | Η ταχύτητα παράδοσης της ομάδας. |
| **Work In Progress (WIP)** | **3 tickets** (102, 103, 104) | Οι εργασίες που είναι "ανοιχτές". Το Kanban στοχεύει σε **χαμηλό WIP** για μείωση context switching. |
| **Cycle Time (Ticket-101)** | Πέμπτη - Τρίτη = **3 ημέρες** | Ο χρόνος που η ομάδα δούλευε ενεργά πάνω στο ticket. |
| **Lead Time (Ticket-101)** | (Αν δημιουργήθηκε 1 εβδομάδα πριν) = **8 ημέρες** | Ο συνολικός χρόνος που περίμενε ο "πελάτης" (από τη δημιουργία του ticket). |

> **Στόχος:** Μείωση **Cycle Time** και **Lead Time** μέσω της διατήρησης **χαμηλού WIP** και της αύξησης του **Throughput**. Τα δεδομένα από το ticketing system είναι ο χρυσός για την ανάλυση της απόδοσης.

<!-- Ας δούμε πώς εφαρμόζονται αυτά τα KPIs στην πράξη, σε μια ομάδα software. Το Kanban board είναι ο πίνακας ελέγχου μας. Κάθε καρτέλα (ticket) είναι μια δουλειά.
Το Throughput είναι απλό: πόσα tickets κλείσαμε αυτή την εβδομάδα; Εδώ, 2.
Το Work In Progress (WIP) είναι πόσα έχουμε ανοιχτά ΤΩΡΑ. Εδώ, 3. Ο χρυσός κανόνας του Kanban είναι 'Stop starting, start finishing' — δηλαδή, κράτα το WIP χαμηλό για να τελειώνεις γρήγορα ό,τι έχεις αρχίσει.
Το Cycle Time για το Ticket-101 είναι 3 μέρες — από τη στιγμή που κάποιος το 'τράβηξε' για δουλειά, μέχρι που τελείωσε.
Το Lead Time είναι ο χρόνος που βλέπει ο πελάτης: από τη στιγμή που ζήτησε κάτι, μέχρι που το πήρε. Περιλαμβάνει και τον χρόνο που το ticket καθόταν στην ουρά (backlog).
Αναλύοντας αυτά τα νούμερα από το Jira, μπορούμε να βρούμε τα bottlenecks μας και να βελτιώσουμε τη ροή. -->
...
| Τύπος | Σύμβολο | Λογική | Παράδειγμα |
|---|:---:|---|---|
| **Exclusive — XOR** | ![w:40](../img/lecture2/gateway_xor.svg) | **ΜΟΝΟ ΕΝΑ** μονοπάτι | «Εγκρίθηκε; ΝΑΙ ή ΟΧΙ» |
| **Parallel — AND** | ![w:40](../img/lecture2/gateway_and.svg) | **ΟΛΑ** τα μονοπάτια ταυτόχρονα | «Πακετάρισμα ΚΑΙ Τιμολόγηση» |
| **Inclusive — OR** | ![w:40](../img/lecture2/gateway_or.svg) | **ΕΝΑ Ή ΠΕΡΙΣΣΟΤΕΡΑ** | «Αν > 1.000€ ΚΑΙ/Ή νέος πελάτης» |
...
Ας μοντελοποιήσουμε μια πραγματική δημόσια διεργασία: την [**«Έκδοση Αντιγράφου Ποινικού Μητρώου»**](https://mitos.gov.gr/index.php/%CE%94%CE%94:%CE%91%CE%BD%CF%84%CE%AF%CE%B3%CF%81%CE%B1%CF%86%CE%BF_%CE%A0%CE%BF%CE%B9%CE%BD%CE%B9%CE%BA%CE%BF%CF%8D_%CE%9C%CE%B7%CF%84%CF%81%CF%8E%CE%BF%CF%85) μέσω του gov.gr, όπως περιγράφεται στο Εθνικό Μητρώο Διαδικασιών (Μίτος).
...
<!-- Ας πάρουμε μια πραγματική διαδικασία από το Εθνικό Μητρώο Διαδικασιών, το mitos.gov.gr, και ας τη 'μεταφράσουμε' σε BPMN. Θα δούμε τη διαδικασία έκδοσης ποινικού μητρώου. Σε αυτή τη διαφάνεια, αναλύουμε τη διαδικασία βήμα-βήμα, αντιστοιχίζοντας κάθε ενέργεια σε έναν ρόλο (Lane) και ένα σύμβολο BPMN. Αυτή η άσκηση μας προετοιμάζει για την επόμενη διαφάνεια, όπου θα δούμε το τελικό διάγραμμα. -->
...
<!--
Ας δούμε τώρα πώς η θεωρία γίνεται πράξη. Αυτό είναι το διάγραμμα BPMN για την έκδοση του ποινικού μητρώου, όπως το περιγράψαμε.

Βλέπουμε τρία Pools: τον Πολίτη, το gov.gr (που είναι το σύστημα) και την Αρμόδια Υπηρεσία.
1. Η διαδικασία ξεκινά από τον Πολίτη (Start Event).
2. Ο Πολίτης υποβάλλει την αίτηση (User Task).
3. Το σύστημα gov.gr την παραλαμβάνει και την προωθεί (Service Task).
4. Ο υπάλληλος στην Αρμόδια Υπηρεσία την ελέγχει (User Task).
5. Εδώ έχουμε την πύλη απόφασης (XOR Gateway): Είναι έγκυρη;
6. Αν 'Όχι', η διαδικασία τερματίζει.
7. Αν 'Ναι', ο υπάλληλος εκδίδει το έγγραφο (User Task).
8. Το σύστημα το στέλνει στη Θυρίδα του Πολίτη (Service Task).
9. Ο Πολίτης το ανακτά και η διαδικασία ολοκληρώνεται με επιτυχία.

Αυτό το διάγραμμα δεν είναι απλώς μια εικόνα. Είναι ένας εκτελέσιμος χάρτης που δείχνει με απόλυτη σαφήνεια ποιος κάνει τι, πότε και υπό ποιες συνθήκες.
-->
...
# Εστίαση: Τι είναι το RPA (Robotic Process Automation);

<!-- _class: casestudy xxsmall -->

Το **RPA** είναι η τεχνολογία που επιτρέπει την αυτοματοποίηση επαναλαμβανόμενων, βασισμένων σε κανόνες εργασιών, χρησιμοποιώντας **«ρομπότ» λογισμικού (bots)** που μιμούνται τις ανθρώπινες ενέργειες πάνω στο **γραφικό περιβάλλον (GUI)** μιας εφαρμογής.

**Πώς λειτουργεί;**
Το bot δεν συνδέεται με APIs. Αντιθέτως, **«βλέπει» την οθόνη** και εκτελεί ενέργειες όπως:
- **Άνοιγμα εφαρμογών** (π.χ. Excel, SAP, browser)
- **Copy-paste δεδομένων** από ένα σύστημα σε άλλο
- **Συμπλήρωση φορμών**
- **Κλικ σε κουμπιά** και μενού
- **Αποστολή email**

**Παράδειγμα:** Ένα bot διαβάζει γραμμή-γραμμή ένα Excel, ανοίγει μια παλιά desktop εφαρμογή λογιστηρίου (που δεν έχει API), και καταχωρεί τα δεδομένα σε πεδία, ακριβώς όπως θα έκανε ένας υπάλληλος.

| | **RPA** | **BPMS** |
|---|---|---|
| **Εστίαση** | Αυτοματοποίηση **Tasks** | Ενορχήστρωση **Processes** |
| **Λογική** | "Doing" — εκτελεί script | "Thinking" — διαχειρίζεται ροή, κανόνες, εξαιρέσεις |
| **Ενσωμάτωση** | Μέσω GUI (επιφανειακά) | Μέσω APIs (σε βάθος) |

> **Συμπέρασμα:** Το RPA είναι ιδανικό για να «κολλήσει» παλιά συστήματα σε νέες διεργασίες. Μέσα σε ένα BPMN διάγραμμα, ένα Task μπορεί να εκτελείται από ένα RPA bot αντί για έναν άνθρωπο.

<!-- Το RPA είναι ο 'ψηφιακός υπάλληλος' που κάνει την πιο βαρετή, επαναλαμβανόμενη δουλειά. Σκεφτείτε το σαν ένα macro που δεν περιορίζεται σε μία εφαρμογή, αλλά μπορεί να δουλέψει σε όλο το desktop σας. Η μεγάλη του δύναμη είναι ότι δεν χρειάζεται APIs. Μπορεί να αυτοματοποιήσει ακόμα και ένα παλιό σύστημα γραμμένο σε COBOL, αρκεί να έχει γραφικό περιβάλλον. Είναι σημαντικό να μην το μπερδεύουμε με το BPMS. Το BPMS είναι ο 'μαέστρος' που ενορχηστρώνει όλη τη διαδικασία. Το RPA είναι ένας από τους 'μουσικούς' που εκτελεί ένα συγκεκριμένο, επαναλαμβανόμενο κομμάτι. -->
...
# Case Study: Camunda & Spring Boot

<!-- _class: casestudy xsmall -->

Πώς η θεωρία της αρχιτεκτονικής BPMS γίνεται πράξη με την **Camunda**, ένα από τα πιο δημοφιλή open-source BPMS.

<div class="columns">
<div>

**Τα Εργαλεία & οι Ρόλοι τους:**

| Εργαλείο | Ρόλος στην Αρχιτεκτονική | Τι Κάνει |
|---|---|---|
| **Camunda Modeler** | **Επίπεδο Μοντελοποίησης** | Γραφικό εργαλείο για τον σχεδιασμό του BPMN διαγράμματος. Αποθηκεύει τη διεργασία σε ένα αρχείο `.bpmn` (XML). |
| **Spring Boot App** | **Μηχανή Εκτέλεσης** | Εφαρμογή Java που ενσωματώνει την Camunda Engine. Διαβάζει το `.bpmn` αρχείο και το "ζωντανεύει". |
| **Java Workers** | **Πάροχοι Πόρων** | Κλάσεις Java που υλοποιούν τη λογική για τα Service Tasks. Η Engine τις καλεί όταν φτάνει στο αντίστοιχο βήμα. |
...
```mermaid
graph TD
    A[Camunda Modeler] -->|Σχεδιασμός| B(process.bpmn);
    B --> C{Εφαρμογή Spring Boot};
    D[Κλήση REST API] -->|Εκκίνηση Διεργασίας| C;
    C -- εκτελεί --> E[User Task];
    C -- καλεί --> F[Java Worker];
    F -- ολοκληρώνει --> C;
```
...
> **Συμπέρασμα:** Το BPMN δεν είναι απλά μια εικόνα. Είναι ένα **εκτελέσιμο μοντέλο** που γεφυρώνει την επιχειρησιακή ανάλυση με τον κώδικα.

<!-- Ας δούμε πώς η θεωρία της αρχιτεκτονικής BPMS γίνεται πράξη με την Camunda. Στο Επίπεδο Μοντελοποίησης έχουμε το Camunda Modeler, ένα desktop εργαλείο όπου σχεδιάζουμε το BPMN διάγραμμά μας. Αυτό αποθηκεύεται ως ένα απλό XML αρχείο. Στη συνέχεια, φτιάχνουμε μια εφαρμογή Spring Boot που ενσωματώνει τη Μηχανή Εκτέλεσης της Camunda. Η εφαρμογή αυτή διαβάζει το BPMN αρχείο και το 'ζωντανεύει'. Όταν ένα Service Task πρέπει να εκτελεστεί, η μηχανή καλεί τον αντίστοιχο Java κώδικα (worker) που έχουμε γράψει. Έτσι, συνδέουμε το οπτικό διάγραμμα με πραγματικό, εκτελέσιμο κώδικα. -->
